...
public class SqList implements IList {
	private GOODS[] listElem; // 线性表存储空间
	...
	public SqList(int maxSize) {
		...
		listElem = new GOODS[maxSize];// 为顺序表分配maxSize个存储单元
	}
	...
	public GOODS get(int i) throws Exception {
	...
	public void insert(int i, GOODS x) throws Exception {
	...
	public void KuoLong(int max){
		if(max<listElem.length){
			System.out.println("改变后长度小于数组原容量");
		}
		else {
			GOODS[] temp=new GOODS[max];
	...
	public void searcher(String search) {
		int count = 0;
		for (int i = 0; i < curLen; i++) {
			if(listElem[i].equals(search))count++;
		}
		System.out.println("表中存在"+count+"个"+search);
	}
	public void jieSuan(){
		if(curLen==0){
